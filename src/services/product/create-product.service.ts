...
export class CreateProductService {
    static async validate(userId: string, name: string, suggestedRetailerPrice: number, origin: string) {
    ...
    static async create(userId: string, name: string, suggestedRetailerPrice: number, origin: string, cost: number) {
        await this.validate(userId, name, suggestedRetailerPrice, origin);
        const sid = await this.getSid();
        const product = new Product({ sid, name, suggestedRetailerPrice, origin, createBy: userId, cost });
