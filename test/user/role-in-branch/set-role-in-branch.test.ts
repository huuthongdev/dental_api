...
import { app, Role, ROOT_NAME, ROOT_EMAIL, ROOT_PHONE, Branch, BranchError, User, UserError, RoleInBranchError } from '../../../src/refs';
...
describe('POST /user/set-role-in-branch/:branchId', () => {
    ...
    beforeEach('Prepare data for test', async () => {
        ...
        branchName = dataInit.masterBranch.name.toString();
    ...
    it('Can set role in current branch for user', async () => {
        ...
        const response = await request(app)
            .put('/user/set-role-in-branch').set({ token }).send(dataSend);
        ...
        const resExpected: any = {
            ...
            roleInBranchs: [{
                ...
                branch: { _id: branchId, sid: 1, name: branchName },
                __v: 0,
                roles: [Role.CHAIRMAN, Role.ACCOUNTANT, Role.CUSTOMER_CARE_MANAGER]
            }],
            isActive: true,
    ...
    it('Cannot set role in branch with removed branch', async () => {
        ...
        const response = await request(app)
            .put('/user/set-role-in-branch').set({ token }).send(dataSend1);
